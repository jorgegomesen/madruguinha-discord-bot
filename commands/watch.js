    ...
    execute(message, watch_url, current_players_data, past_players_data) {
                ...
                for (let it = 0; it < ph_len; it++) {
                    ...
                    if (hist_build_str != null) {
                        ...
                        * */
                        ...
                        if(hist_build_str.indexOf('MINE') != -1)
                            continue;
        ...
        function main() {
            ...
            for (let key of keys) {
                ...
                if (diff < 0) {
                    final_string += string + '- [ demolished ]\n';
                    demolished = true;
                    //play_alert_module.execute(message, 'demolido');
                }
                ...
                if (!demolished) {
                    ...
                    if (buildings_possibilites) {
                        ...
                        let string_aux = `[${buildings_possibilites.join(' or ')}] ${time}`;

                        final_string += demolish_css + (demolished ? '' : string);

                        if(!demolished)
                          final_string += string_aux; 
                ...
                if (diff >= 512 && diff < 530) {
                    final_string += demolished ? '' : string;
                    final_string += demolish_css + '[ ACADEMY 1 ]\n';
                    updatePastPlayersHistory(key, `${demolish_css}[ ACADEMY 1 ] ${time}`);
                    play_alert_module.execute(message, 'academia');
                }
                demolished = false;
            }
            ...
            let speed_info = '** Speed being watched => ' + watch_url + ' **\n\n';

            if(final_string.length){
              message.channel.send('```diff\n' + (speed_info + final_string) + '```');
            }
        }
